...
def move_robot(the_distance_left, the_distance_right, the_distance_y, theta):
    ...
    if math.fabs(math.fabs(theta) - 90) <= 2:
        ...
        action_append("forwardSlow0403")
        count_forward+=1
        if count_forward > 5+the_distance_Y/5:
            action_append("forwardSlow0403")
            action_append("Stand")
            is_bridge_done = 1
...
def pass_bridge():
    ...
    while True:
        ...
        ChestOrg_copy = ChestOrg_copy[100:540, 100:380]
        ...
        if Chest_areaMaxContour is not None:
            ...
            d1 = 240 - Chest_center_x
            d2 = 640 - Chest_bottom_center_y
            if math.fabs(Chest_top_right[1] - Chest_top_left[1]) < 5:
                theta = 90
            else:
...
def pass_door():
    global HeadOrg_img
    step_door_flag = 0 # 进入第几阶段（0：正面朝门；1：盲走）
    step_contour = 0 # 计算步数
    j = 0 # 拍照的标志位
    Chest_top_center_y = 480 # 找轮廓方式的选择标志位
    ...
    while True:
        ...
        cv2.putText(HeadOrg_img_draw, "top_center_y:" + str(int(Chest_top_center_y)), (30, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 255), 2)
        cv2.putText(HeadOrg_img_draw, "top_center_x:" + str(int(Chest_top_center_x)), (30, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 255), 2)
        cv2.putText(HeadOrg_img_draw, "bottom_center_y:" + str(int(Chest_bottom_center_y)), (30, 260), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 255), 2)
        cv2.putText(HeadOrg_img_draw, "bottom_center_x:" + str(int(Chest_bottom_center_x)), (30, 290), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 255, 255), 2)
        ...
        if step_door_flag == 0:
            ...
            if Chest_bottom_center_y < 470:
                ...
                if Chest_top_bottom <= 20 and math.fabs(Chest_bottom_angle) <= 5:
                    print("角度可了00")
                    action_append("Forwalk02")
            ...
            if Chest_bottom_center_y >= 470:
                ...
                if math.fabs(Chest_bottom_angle) <= 5:
                    print("角度可了0")
                elif  max_box[3][1] > max_box[2][1]:
                    print("左转一点点1")
                    action_append("turn001L")
                ...
                if math.fabs(Chest_bottom_center_x - 320) < 20:
                    print("左右可了01")
                    
                    if Chest_top_center_y > 5:
                        action_append("Forwalk00")
                    if Chest_top_center_y <= 5:
                        step_door_flag = 1
    
                elif Chest_bottom_center_x > 320:
                    print("往右一点点")
                    action_append("Right02move")
                else:
                    print("往左一点点")
...
def act_move():
    ...
    if True:
        if step == 0:  # 发现球,前进到球跟前
                ...
                if ball_x > 240:
                    ...
                    if angle_flag == True:
                        ...
                        if angle > 78 or angle < -88:
                            ...
                            action_append('turn001L')
                            print('step5 200<ball_x<240 -88<angle<-65 turn001L', angle)
                        elif -65 < angle < 0:
                            action_append('turn003L')
            ...
            if ball_y < 420:
                action_append('Forwalk00')
                print('step6 ball_y<420 Forwalk00', ball_y)
            elif ball_y > 470:
                action_append('Back2Run')
                ...
                if ball_x > 220:
                    action_append('Right02move')
                    print('step6 420<ball_y<470 ball_x>220 Right02move', ball_x)
                elif ball_x < 200:
                    action_append('Left02move')
                    print('step6 420<ball_y<470 ball_x<200 Left02move', ball_x)
                elif 200 < ball_x < 220:

                    if angle_flag == True:

                        if angle > 79 and angle < 88:
                            step = 7
                            print('from step6 go to step7', angle)
                            cv2.imwrite('6->7.jpg', chest)
                        elif 65 < angle < 79:
                            action_append('turn001R')
                            print('step6 200<ball_x<220 65<angle<79 turn001R', angle)
                        elif 0 < angle < 65:
                            action_append('turn003R')
                            print('step6 200<ball_x<220 0<angle<65 turn003R', angle)
                        elif angle < -65 or angle>88:
                            action_append('turn001L')
                            print('step6 200<ball_x<220 angle < -65 or angle>88 turn001L', angle)
                        elif -65 < angle < 0:
                            action_append('turn003L')
