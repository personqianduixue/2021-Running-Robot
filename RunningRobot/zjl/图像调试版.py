...
color_range = {
    ...
    #'green_bridge_1': [(45, 75, 40), (90, 255, 210)],
    'green_bridge_1':[(52,106,109),(90,255,255)],
    #'blue_bridge_1': [(78, 43, 46), (124, 255, 255)],
    'blue_bridge_1':[(83,86,135),(122,255,255)],
    'blue' :[(95, 43, 46), (124,255, 255)],
...
##################################################第八关：过洞###############################################
def square_hole():
    global ChestOrg_copy, state, state_sel, step, reset, skip, debug, FLAG
    ...
    while state == 8:  # 初始化
        ...
        if True:  # head发现黄色区域
            ...
            #ChestOrg_copy = ChestOrg_copy[250:600]
            ...
            '''def nothing():
                pass
            pic = np.zeros((640,480,3),dtype=np.uint8)
            ...
            imgray = cv2.cvtColor(ChestOrg_copy,cv2.COLOR_BGR2HSV)
            cv2.imshow("frame",ChestOrg_copy)
            ...
            img = cv2.add(ChestOrg_copy,pic,mask=mask)
            cv2.imshow("img",img)
            ...
            '''if FLAG == 2 or 7 or 12:
                frame = cv2.inRange(frame_hsv, color_range['blue_bridge_1'][0],
                                    color_range['blue_bridge_1'][1])
            else:   
                frame = cv2.inRange(frame_hsv, color_range['green_bridge_1'][0],
                                    color_range['green_bridge_1'][1])'''
            frame = cv2.inRange(frame_hsv, color_range['blue_bridge_1'][0],
                                    color_range['blue_bridge_1'][1])
            ...
            for c in cntskeng:
                ...
                if (edge2 != 0) and (edge3 != 0):
                    ...
                    tiaojian2 = ((ratio > 0.25) and (ratio < 4) and (area_temp > 1000) and (area_temp < 50000) and (
                            step <= 2))
        ...
        if action_DEBUG == False:
            if step == 0:  # 依据yellow调整左右位置  到接触黑色
                ...
                # x 225 # 240
                    ...
                    if bottom_center_x > 245:  # 大右移
                        print("Black_bottom_center_x > 0.54 大右移 Right02move，当前中心点x坐标为：  ", bottom_center_x)
                        #action_append("Right02move")
                    else:
                        print("Black_bottom_center_x > 0.54 小右移 Right1move，当前中心点x坐标为：   ", bottom_center_x)
                        #action_append("Right1move")
                elif bottom_center_x < 215:  # 小左移 230.4
                    if bottom_center_x < 205:  # 大左1移
                        print("Black_bottom_center_x < 0.48 大左移 Left02move，当前中心点x坐标为：   ", bottom_center_x)
                        #action_append("Left02move")
                    else:
                        print("Black_bottom_center_x < 0.48 小左移 Left1move，当前中心点x坐标为：   ", bottom_center_x)
                        #action_append("Left1move")
                ...
                if angle_bottom < 43.0 :  # 右转
                    if angle_bottom < 39.0 :  # 大右转
                    ...
                    # time.sleep(1)   # timefftest
                elif angle_bottom > 47.0 :  # 左转
                    if angle_bottom > 51.0  :  # 大左转
